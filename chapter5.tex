...
In table \ref{table:p_test_list4} we note that many of the keywords used leads to very few total hits. In particular the keywords Java and Prisoners Dilemma returns less than 10 results, making the top 10 test a bit inaccurate. Having so few total hits is reflected in the precision also dropping quite low. In figure \ref{fig:avgPrecision} we can see that the white list MathTechWiki separates itself from the rest of the lists with almost 50 percent worse precision. This big loss of precision indicates that the white list excludes to many relevant examples from index's corpus.
