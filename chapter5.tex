...
To further explore the systems ability of retrieving relevant examples, we will test the system's recall. While precision is a measure of exactness, recall can be seen as a measure of completeness. Recall is calculated as follows:
\[\frac{|R \cap E |}{|R|}\]
In the equation above \(R\) is the set of all relevant examples and \(E\) is the set of all the retrieved examples. If no relevant examples are returned the recall is 0, and if all the relevant examples are retrieved the recall is 1. To properly measure recall all the examples have been manually inspected, to decide which examples are relevant or not relevant for a search with a specific keyword. The three keywords \textit{java}, \textit{algebra} and \textit{nash equilibrium} was chosen.
