...
\includegraphics[width=\textwidth]{wq_empty}
\label{fig:wq_empty}
\end{figure}
...
When querying by examples, relevant examples which can also assist in the learning of the subject, are expected to be returned. To evaluate how the system accomplishes this, we will make us of knowledge from section \ref{5:keywords}. The knowledge will be used to create an optimal environment, by choosing the best performing white list and a keyword with perfect precision. The white list \textit{MathTech} performed best for the top ten results with an average precision of 0.72, and will therefor be the white list chosen. As keyword, \textit{Java} will be used for mainly three reasons. First, it is on of the keywords with a perfect score when \textit{MathTech} was applied as white list. Second, \textit{Java} is a search phrase, but at the same time not the name of a category. Finally, it is very easy to consistently judge whether an example is about java or not, since it often contains java code.

When testing there are two results that will be considered. First, how many relevant examples are shown together with the main example. Second, how many of the examples are relevant. The first ten examples will be used to test these two results.
%finn et annet ord en result.
